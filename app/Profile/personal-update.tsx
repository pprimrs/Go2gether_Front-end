...
export default function PersonalUpdateScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (res.status === 404) {
          // เผื่อโดนยิงตรงๆ มาที่ update แต่ไม่เคยมีโปรไฟล์ → ส่งไปหน้า create
          if (!cancelled) router.replace("/Profile/personal-create");
          return;
        }
        let json: any = {};
        try { json = await res.json(); } catch {}
        if (!res.ok) {
          if (!cancelled) Alert.alert("Error", json?.message || `Load profile failed (HTTP ${res.status}).`);
          return;
        }
        ...
        if (!cancelled) {
          setForm({
            ...
            foodAllergies: user?.allergic_food ?? "-",
            allergicDrugs: user?.allergic_drugs ?? "-",
            chronicDisease: user?.chronic_disease ?? "-",
            emergencyCall: user?.emergency_contact ?? "",
          });
        }
      } catch {
        if (!cancelled) Alert.alert("Error", "Could not load profile.");
      } finally {
  ...
  const onSave = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        last_name: "",
        phone: "",
        // username: อัปเดตหรือไม่อัปเดตแล้วแต่ backend;
      ...
      if (!res.ok) {
        Alert.alert("Error", json?.message || `Update profile failed (HTTP ${res.status}).`);
        return;
      }
      ...
      Alert.alert("Saved", json?.message || "Your personal information has been updated.");
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.select({ ios: "padding" })}>
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <Text style={styles.screenTitle}>Update Personal Information</Text>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.item}>
            ...
            <TextInput
              style={styles.input}
              placeholder="Food Name (or - if none)"
              placeholderTextColor="#B9B9B9"
          ...
          <View style={styles.item}>
            ...
            <TextInput
              style={styles.input}
              placeholder="Drugs Name (or - if none)"
              placeholderTextColor="#B9B9B9"
          ...
          <View style={styles.item}>
            ...
            <TextInput
              style={styles.input}
              placeholder="Medical Condition (or - if none)"
              placeholderTextColor="#B9B9B9"
